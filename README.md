...
This repo publishes two versions of the dashboard from the same data pipeline:

- **`/`** (root) — the static-baked dashboard. Scoring happens in Python; the
  HTML is regenerated by the GitHub Actions cron with Spencer's preferences
  baked in. Fast and simple, but every visitor sees the same picks.

- **`/v2/`** — browser-side filtering. Every visitor gets their own preferences
  (saved to `localStorage`), can adjust category weights / brand list / kid-age
  filters / price tolerance from a settings drawer, can star events to
  bookmark, and can export events to their calendar (.ics or Google Calendar).
  Same data, same look — just customizable.

The pipeline writes both: `docs/index.html` for v1 and `docs/v2/events.json`
for v2 (loaded by `docs/v2/index.html` + `docs/v2/app.js`). Choose your
adventure or share both URLs.
...
- `render.py` — v1 HTML template.
- `validate_urls.py` — parallel URL health-check + per-source fallback.
- `v2_emit.py` — emits the rich `docs/v2/events.json` for browser-side
  filtering (no scoring; just match flags + defaults).
- `main.py` — orchestrates fetch → score → render → write `docs/`.
- `.github/workflows/update.yml` — cron + push to refresh.
- `docs/index.html` — v1 published page (regenerated by the pipeline).
- `docs/v2/index.html`, `app.js`, `events.json` — v2 customizable page.
